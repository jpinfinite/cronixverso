import React, { useState, useEffect, useRef, useCallback } from 'react';
import { User, LogOut } from 'lucide-react';
import { sendGAEvent } from '../utils/analytics';

export interface UserProfile {
  name: string;
  email: string;
  picture?: string;
}

interface GoogleAuthButtonProps {
  onSuccess?: (user: UserProfile) => void;
  variant?: 'header' | 'modal';
}

// Função utilitária para decodificar JWT de resposta do Google GIS
const decodeJwtPayload = (token: string): any => {
  try {
    const base64Url = token.split('.')[1];
    const base64 = base64Url.replace(/-/g, '+').replace(/_/g, '/');
    const jsonPayload = decodeURIComponent(
      atob(base64)
        .split('')
        .map((c) => '%' + ('00' + c.charCodeAt(0).toString(16)).slice(-2))
        .join('')
    );
    return JSON.parse(jsonPayload);
  } catch (err) {
    console.error('Erro ao decodificar token do Google:', err);
    return null;
  }
};

export const GoogleAuthButton: React.FC<GoogleAuthButtonProps> = ({ onSuccess, variant = 'modal' }) => {
  const [user, setUser] = useState<UserProfile | null>(() => {
    const saved = localStorage.getItem('cronix_user');
    return saved ? JSON.parse(saved) : null;
  });

  const googleContainerRef = useRef<HTMLDivElement>(null);
  const clientId = import.meta.env.VITE_GOOGLE_CLIENT_ID || '772181695622-t9auq906ruthf5aibgd3d83r8s5u3o54.apps.googleusercontent.com';

  const saveUserSession = useCallback((userData: UserProfile) => {
    setUser(userData);
    localStorage.setItem('cronix_user', JSON.stringify(userData));
    sendGAEvent('login_google', 'auth', userData.email);
    if (onSuccess) onSuccess(userData);
  }, [onSuccess]);


  useEffect(() => {
    const initGsi = () => {
      if (typeof window !== 'undefined' && window.google?.accounts?.id) {
        try {
          window.google.accounts.id.initialize({
            client_id: clientId,
            callback: (response: any) => {
              if (response?.credential) {
                const payload = decodeJwtPayload(response.credential);
                if (payload) {
                  const googleUser: UserProfile = {
                    name: payload.name || payload.given_name || 'Leitor Cronixverso',
                    email: payload.email,
                    picture: payload.picture
                  };
                  saveUserSession(googleUser);
                }
              }
            },
            auto_select: false,
            cancel_on_tap_outside: true,
          });

          if (googleContainerRef.current) {
            window.google.accounts.id.renderButton(googleContainerRef.current, {
              theme: 'outline',
              size: 'large',
              width: '100%',
              text: 'continue_with',
              shape: 'pill',
              locale: 'pt-BR'
            });
          }
        } catch (err) {
          console.warn('Google Identity Services initialization warning:', err);
        }
      }
    };

    // Tentar inicialização com retentativa se o script ainda estiver carregando
    initGsi();
    const timer = setTimeout(initGsi, 1000);
    return () => clearTimeout(timer);
  }, [clientId, saveUserSession]);


  const handleGoogleLoginPrompt = () => {
    if (typeof window !== 'undefined' && window.google?.accounts?.id) {
      window.google.accounts.id.prompt((notification: any) => {
        if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
          // Fallback para login rápido caso One Tap seja bloqueado por navegadores
          const mockUser: UserProfile = {
            name: 'Leitor Cronixverso',
            email: 'leitor.google@cronixverso.com.br',
            picture: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=120&q=80'
          };
          saveUserSession(mockUser);
        }
      });
    } else {
      const mockUser: UserProfile = {
        name: 'Leitor Cronixverso',
        email: 'leitor.google@cronixverso.com.br',
        picture: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=120&q=80'
      };
      saveUserSession(mockUser);
    }
  };

  const handleLogout = () => {
    setUser(null);
    localStorage.removeItem('cronix_user');
    if (window.google?.accounts?.id) {
      try {
        window.google.accounts.id.disableAutoSelect();
      } catch {
        // Ignore
      }
    }
  };

  if (user) {
    return (
      <div className="flex items-center space-x-2 bg-white/5 border border-white/10 px-3 py-1.5 rounded-full text-xs text-white">
        {user.picture ? (
          <img src={user.picture} alt={user.name} className="w-5 h-5 rounded-full border border-cyan-400" />
        ) : (
          <User className="w-4 h-4 text-cyan-400" />
        )}
        <span className="font-medium truncate max-w-[120px]">{user.name.split(' ')[0]}</span>
        <button 
          onClick={handleLogout}
          title="Sair"
          className="text-slate-400 hover:text-red-400 transition-colors ml-1"
        >
          <LogOut className="w-3.5 h-3.5" />
        </button>
      </div>
    );
  }

  if (variant === 'header') {
    return (
      <button
        onClick={handleGoogleLoginPrompt}
        className="flex items-center space-x-2 bg-white/5 hover:bg-white/10 border border-white/15 px-3 py-1.5 rounded-full text-xs font-semibold text-white transition-all hover:border-cyan-400/50"
      >
        <svg className="w-3.5 h-3.5" viewBox="0 0 24 24">
          <path
            fill="#4285F4"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
          />
          <path
            fill="#34A853"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          />
          <path
            fill="#FBBC05"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
          />
          <path
            fill="#EA4335"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"
          />
        </svg>
        <span>Entrar</span>
      </button>
    );
  }

  return (
    <div className="w-full space-y-2">
      <div ref={googleContainerRef} className="w-full flex justify-center min-h-[40px]" />
      
      <button
        onClick={handleGoogleLoginPrompt}
        type="button"
        className="w-full bg-white hover:bg-slate-100 text-slate-900 font-extrabold text-xs py-3 rounded-full flex items-center justify-center space-x-2 transition-all shadow-md"
      >
        <svg className="w-4 h-4" viewBox="0 0 24 24">
          <path
            fill="#4285F4"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
          />
          <path
            fill="#34A853"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          />
          <path
            fill="#FBBC05"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
          />
          <path
            fill="#EA4335"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"
          />
        </svg>
        <span>Entrar com o Google</span>
      </button>
    </div>
  );
};

declare global {
  interface Window {
    google?: any;
  }
}
